...
Thanh Tra pre-scan chạy audit tool thật khi có sẵn (`npm audit`, `pnpm audit`, `pip-audit`, `cargo audit`). Khi không chạy được hoặc offline, rule này:
1. Flag một số package + version **well-known vulnerable** (static list dưới)
2. Ghi audit gap rõ và khuyến nghị user chạy tool phù hợp (`npm audit` / `pip-audit` / `cargo audit` / `govulncheck` / `composer audit`)

## Khi nào HIGH

- Package nằm trong static list dưới với version vulnerable
- `package-lock.json` / `requirements.txt` / `go.sum` / `composer.lock` / `Cargo.lock` show version cụ thể có CVE
...
1. **Đọc lockfile**: `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `requirements.txt`, `Pipfile.lock`, `poetry.lock`, `go.sum`, `composer.lock`, `Gemfile.lock`, `Cargo.lock`
