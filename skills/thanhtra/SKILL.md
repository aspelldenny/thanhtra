...
Quét lỗ hổng bảo mật cho code do AI sinh ra (vibe code). Thanh Tra kế thừa bộ rule MIT từ vbsec upstream, giữ credit tác giả gốc, và bổ sung CLI/core deterministic để agent bớt phụ thuộc reasoning cơ học. Bộ skill này check 24 lỗi bảo mật phổ biến nhất của vibe code, kế thừa kiến trúc SMALL/LARGE mode từ bộ rule production của SePay, tổng quát hóa cross-language (mặc định) + chuyên sâu cho Go/PHP/TypeScript/Python/Rust/Swift/Shell.
...
│                                                                      │
│   Both apply:                                                        │
│     - rules/generic/*.md (24 rules, luôn chạy)                       │
...
├── rules/
│   ├── generic/                      # 24 rules cross-language (bắt buộc apply)
