...
| `verdict_pass` | ĐẠT |
| `verdict_warn` | SECURITY GATE CẢNH BÁO |
| `verdict_fail` | SECURITY GATE FAIL |
| `verdict_pass_desc` | Không phát hiện lỗi CRITICAL/HIGH trong phạm vi scan. Đây không phải chứng nhận hệ thống an toàn tuyệt đối. |
| `verdict_warn_desc` | Có finding mức CAO theo rule Thanh Tra. Reviewer cần triage trước khi quyết định ship/deploy. |
| `verdict_fail_desc` | Có finding mức NGHIÊM TRỌNG theo rule Thanh Tra. Đây là trạng thái fail của security gate, không phải kết luận chất lượng tổng thể của app. |
| `verdict_scope_note` | Verdict chỉ phản ánh policy của Thanh Tra trong phạm vi scan hiện tại; không thay thế audit đầy đủ, pentest, hay quyết định release của owner. |
...
| `msg_scope_needs_git` | Scope này yêu cầu git repository. Dùng skill không có scope (mặc định scan toàn bộ folder), hoặc khởi tạo git rồi gọi lại. Tip: `git init && git add .` |
| `msg_no_git_note` | ⚠️ Folder không phải git repository — đã quét tất cả file trong folder. Không bỏ qua theo `.gitignore` (vì không có git). |
...
| `footer_repo` | https://github.com/aspelldenny/thanhtra |
...
| `msg_gitignore_warning_text` | Thư mục `thanhtra-reports/` chưa có trong `.gitignore`. Để tránh commit báo cáo vào Git, thêm dòng `thanhtra-reports/` vào `.gitignore`. |
| `msg_reports_dir_created` | Đã tạo thư mục `thanhtra-reports/` để lưu báo cáo |
