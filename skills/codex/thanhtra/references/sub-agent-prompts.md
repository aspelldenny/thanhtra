...
# Deterministic pre-scan evidence for this chunk
These are signals from `scripts/thanhtra-pre-scan.py`. They are NOT final findings.
Start with them, then read full context and apply L1-L4 reasoning.
...
Agent({
  ...
  description: "Thanh Tra scan chunk: api/handlers",
