...
**Layout:**
1. Header block (scope, file count, primary lang, mode, date, inspector, lang code) — {header_inspector} = tên+phiên bản model đang chạy scan (vd Claude Opus 4.8, GPT-5.5 Codex); tự khai theo model thực tế
...
| Resume on interrupt | TodoWrite tasks | `.thanhtra-tmp/findings-*.md` (re-run skip chunk đã có file) |
