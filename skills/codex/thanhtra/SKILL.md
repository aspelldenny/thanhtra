---
name: thanhtra
description: Use when scanning code for security vulnerabilities. Use when user says "scan security", "kiểm tra bảo mật", "security audit", "review security", or invokes `/thanhtra`. For large scans (>20 main-language files OR >30 total OR >14 days) processes chunks sequentially. Outputs bilingual reports (vi/en).
license: MIT
---

# Thanh Tra — Security Scanner cho Vibe Coders (Codex CLI variant)

Quét lỗ hổng bảo mật cho code do AI sinh ra (vibe code). Thanh Tra kế thừa bộ rule MIT từ vbsec upstream, giữ credit tác giả gốc, và bổ sung CLI/core deterministic để agent bớt phụ thuộc reasoning cơ học. Bộ skill này check 21 lỗi bảo mật phổ biến nhất của vibe code, kế thừa kiến trúc SMALL/LARGE mode, tổng quát hóa cross-language + chuyên sâu cho Go/PHP/Python/TypeScript.

> Repo: https://github.com/aspelldenny/thanhtra
> License: MIT
> **Platform:** OpenAI Codex CLI. Phiên bản Claude Code spawn parallel sub-agents; phiên bản này dùng **sequential chunking** để giữ portability — chậm hơn ~3× nhưng output identical.
...
```
/skills              # mở skill picker, chọn thanhtra
$thanhtra   # invoke trực tiếp qua $-prefix
```
...
```
┌─────────────────────────────────────────────────────────────────────┐
│            Thanh Tra SCAN WORKFLOW (Codex — Sequential)                  │
├─────────────────────────────────────────────────────────────────────┤
...
- Gom hot spots theo 21 canonical rule IDs
...
- **LARGE mode:** Read [`workflows/large-review-sequential.md`](workflows/large-review-sequential.md) — chunk + xử lý **tuần tự** (Codex hiện chưa có cơ chế spawn parallel sub-agents từ skill; sequential giữ logic chunking + aggregation nhưng chạy 1 chunk/lần)
...
**Save-to-file:** ghi TOÀN BỘ report (identical với stdout) vào `thanhtra-reports/scan-<timestamp>.md` dùng tool write/create file của Codex.

Sau đó in 1-2 dòng note ra stdout:
```
📄 {msg_report_saved}: thanhtra-reports/scan-<timestamp>.md
...
| Aspect | Claude Code | Codex (file này) |
|---|---|---|
| LARGE mode | Parallel sub-agents (3 cùng lúc) | Sequential chunking (1 chunk/lần) |
| Resume on interrupt | TodoWrite tasks | `.Thanh Tra-tmp/findings-*.md` (re-run skip chunk đã có file) |
| Frontmatter | `userInvocable: true` | `license: MIT` |
| Invocation | `/thanhtra` | `/skills` → pick, hoặc `$thanhtra`, hoặc auto-match |

Toàn bộ rules, i18n, output format, language detection — identical với Claude Code variant. Khi update rule → sửa ở canonical (`skills/thanhtra/`) → chạy `./scripts/sync-skills.sh` để propagate.
