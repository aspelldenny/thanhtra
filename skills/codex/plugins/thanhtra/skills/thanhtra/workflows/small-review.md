...
3. **Read pre-scan evidence (BẮT BUỘC):**
   - Đọc `.thanhtra-pre-scan.json`
   - Tạo priority list từ:
     - `secret_hits_masked`
     - `git_secret_signals`
     - `docker_exposures`
     ...
     - `dependency_audits`
     - `hotspots_by_rule`
   - Scan code theo priority này trước, sau đó mới rà phần còn lại.

Pre-scan evidence chỉ là tín hiệu. Không render finding từ evidence nếu chưa đọc context hoặc chưa xác nhận bằng rule reasoning, trừ dependency audit tool output đã rõ CVE/package/version.
...
- **OUTDATED-DEPENDENCY**: ưu tiên `dependency_vulnerabilities` trong `.thanhtra-pre-scan.json`. Nếu có `dependency_warnings` (vd unmaintained/notice), cân nhắc MEDIUM/LOW supply-chain finding theo exploitability. Nếu rỗng nhưng audit tool có stdout, đọc `dependency_audits`. Nếu tool missing/fail, đọc `package.json` / `requirements.txt` / `go.mod` / `composer.json` / `Gemfile.lock` / `Cargo.lock` → check static list và ghi audit gap.
