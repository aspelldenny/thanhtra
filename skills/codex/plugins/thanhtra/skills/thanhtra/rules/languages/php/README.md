...
These rule files override generic rules when Thanh Tra detects the primary language as PHP.

## How override works

When Thanh Tra scans a repo and detects primary language `php`, for each rule id present in BOTH `rules/generic/<id>.md` AND `rules/languages/php/<id>.md`, the language-specific version REPLACES the generic. Generic rules without a language override apply as-is.
...
5. Test bằng `/thanhtra` trên PHP repo có vulnerability đó
