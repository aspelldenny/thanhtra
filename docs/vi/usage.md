...
3. Skill sẽ thấy thư mục `.Thanh Tra-tmp/` còn findings cũ → tiếp tục từ chunk chưa xong

**Quan trọng:**
- ĐỪNG xóa `.Thanh Tra-tmp/` thủ công nếu muốn resume
- Sau khi scan xong, Thanh Tra tự cleanup `.Thanh Tra-tmp/`
- Thêm `.Thanh Tra-tmp/` vào `.gitignore` của repo nếu chưa có:
  ```
  .Thanh Tra-tmp/
  ```
...
Thanh Tra là Claude Code skill — chạy trong agent CLI. Có 2 cách integrate:
